...
| Sub-phase 3G: aarch64 port                  | record + Tier 2 work cross-arch | data/syscall_aarch64.tbl (step 66); regs_aarch64 + UserRegsAarch64 + PTRACE_GETREGSET/SETREGSET (step 102); record_session arch dispatch + Linux build hygiene (step 103); proc_regs Tier 2 register-restore on aarch64 + cross-arch hygiene (step 105). All five Phase 5 crates + tests now cross-compile to aarch64-unknown-linux-gnu and the existing `test-arm64` CI job covers them. Aarch64 instruction-trap classifier (CNTVCT_EL0 / MRS / AT) still pending. |
