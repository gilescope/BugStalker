...
| Sub-phase 3D: non-deterministic instrs      | recorder + vDSO patcher      | step_until_event emits Event::InstructionTrap on SIGSEGV/SIGILL at a classified PC; advances RIP past the trap. Step 78 lands the cross-process vDSO patcher (PTRACE_POKEDATA via FOLL_FORCE); replay-side RAX rewrite still TODO. |
