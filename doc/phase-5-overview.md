...
| Sub-phase 3D: non-deterministic instrs      | recorder + vDSO patcher + opt-ins | step_until_event emits Event::InstructionTrap on SIGSEGV/SIGILL at a classified PC; advances RIP past the trap. Step 78 lands the cross-process vDSO patcher (PTRACE_POKEDATA via FOLL_FORCE). Step 86: RecordOptions::patch_vdso + trap_tsc opt-in flags; step 88: replay-record --patch-vdso / --trap-tsc CLI exposure. Replay-side RAX rewrite still TODO. |
| Sub-phase 3E: signals                       | record + best-effort replay  | step_until_event emits Event::Signal on SignalDelivery via PTRACE_GETSIGINFO; replay_program delivers via kill(2) (not PC-precise; PTRACE_SETSIGINFO is the precision upgrade). |
