...
| Sub-phase 3G: aarch64 port                  | regs + dispatch primitives   | data/syscall_aarch64.tbl (step 66); regs_aarch64 module + UserRegsAarch64 + PTRACE_GETREGSET/SETREGSET + call_frame extraction (step 102); record_session arch dispatch + Linux build hygiene (step 103). Full aarch64 record_session (instruction-trap classifier, Tier 2 register-restore) still pending. |
