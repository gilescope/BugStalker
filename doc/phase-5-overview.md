...
| Sub-phase 3D: non-deterministic instrs      | record + replay both wired   | step_until_event emits Event::InstructionTrap; vDSO patcher + opt-ins; PTRACE_SETREGS replays the recorded result into RAX/EDX:EAX/EBX/ECX (step 97); ReplayOptions::patch_vdso (step 98). |
| Sub-phase 3E: signals                       | record + content-precise replay | step_until_event emits Event::Signal via PTRACE_GETSIGINFO; replay path: kill(2) (non-ptraced, best-effort) or PTRACE_SETSIGINFO (ptraced, content-precise). PC-precise via single-step rendezvous still queued. |
