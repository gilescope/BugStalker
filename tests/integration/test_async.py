...
class CommandTestCase(unittest.TestCase):
    def test_runtime_info_1(self):
        for binary in tokio_binaries():
            self.debugger = Debugger(
                path=f"./examples/tokio_tcp/{binary}/target/debug/{binary}"
            )
    ...
    def test_step_over(self):
        ...
        for binary in tokio_binaries():
            self.debugger = Debugger(
                path=f"./examples/tokio_vars/{binary}/target/debug/{binary}"
            )
            self.debugger.cmd("break main.rs:29")
            self.debugger.cmd("run")
            self.debugger.cmd_re(
                "async next", r"Task id: \d", r"30     let _b = inner_1"
            )
            self.debugger.cmd_re(
                "async next", r"Task id: \d", r"32     tokio::time::sleep"
            )
            ...
            self.debugger.cmd_re(
                "async next", r"Task id: \d", r"33     let _c = inner_1"
            )
