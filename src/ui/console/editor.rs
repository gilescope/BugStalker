...
use crate::ui::command::parser::{
    ARG_ALL_KEY, ARG_COMMAND, ARG_DEBUG_COMMAND, ASYNC_COMMAND,
    ASYNC_COMMAND_AWAIT_TRACE_SUBCOMMAND, ASYNC_COMMAND_AWAIT_TRACE_SUBCOMMAND_SHORT,
    ASYNC_COMMAND_BACKTRACE_SUBCOMMAND, ASYNC_COMMAND_BACKTRACE_SUBCOMMAND_SHORT,
    ASYNC_COMMAND_STEP_OUT_SUBCOMMAND, ASYNC_COMMAND_STEP_OUT_SUBCOMMAND_SHORT,
    ASYNC_COMMAND_STEP_OVER_SUBCOMMAND, ASYNC_COMMAND_STEP_OVER_SUBCOMMAND_SHORT,
    ASYNC_COMMAND_TASK_SUBCOMMAND, BACKTRACE_ALL_SUBCOMMAND, BACKTRACE_COMMAND,
    BACKTRACE_COMMAND_SHORT, BREAK_COMMAND, BREAK_COMMAND_SHORT, CALL_COMMAND, CONTINUE_COMMAND,
    CONTINUE_COMMAND_SHORT, FRAME_COMMAND, FRAME_COMMAND_INFO_SUBCOMMAND,
    FRAME_COMMAND_SWITCH_SUBCOMMAND, HELP_COMMAND, HELP_COMMAND_SHORT, MEMORY_COMMAND,
    MEMORY_COMMAND_READ_SUBCOMMAND, MEMORY_COMMAND_SHORT, MEMORY_COMMAND_WRITE_SUBCOMMAND,
    ORACLE_COMMAND, REGISTER_COMMAND, REGISTER_COMMAND_INFO_SUBCOMMAND,
    ...
    SOURCE_COMMAND, SOURCE_COMMAND_DISASM_SUBCOMMAND, SOURCE_COMMAND_FUNCTION_SUBCOMMAND,
    ...
    TRIGGER_COMMAND_WP_TRIGGER_SUBCOMMAND, VAR_COMMAND, VAR_DEBUG_COMMAND, VAR_LOCAL_KEY,
    ...
    fn complete(
        ...
        match CompletableCommand::recognize(line) {
            ...
            Some(CompletableCommand::PrintVariables(maybe_var)) => {
                ...
                let variants: Vec<Vec<u8>> = self.var_hints.predictive_search(maybe_var).collect();
                if !variants.is_empty() {
                    let variants_iter = variants.iter().map(|var| {
                        std::str::from_utf8(var.as_slice()).expect("invalid utf-8 string")
                    });
            ...
            Some(CompletableCommand::PrintArguments(maybe_arg)) => {
                ...
                let variants: Vec<Vec<u8>> = self.arg_hints.predictive_search(maybe_arg).collect();
                if !variants.is_empty() {
                    let variants_iter = variants.iter().map(|var| {
                        std::str::from_utf8(var.as_slice()).expect("invalid utf-8 string")
                    });
        ...
        let commands = [
            ...
            CommandHint {
                ...
                subcommands: vec![
                    ...
                    ASYNC_COMMAND_AWAIT_TRACE_SUBCOMMAND.to_string(),
                    ASYNC_COMMAND_AWAIT_TRACE_SUBCOMMAND_SHORT.to_string(),
                ],
            },
            CALL_COMMAND.into(),
            CommandHint {
